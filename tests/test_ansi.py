...
class TestColorizeThreshold:
    def test_less_below_threshold_is_green(self) -> None:
        result = colorize_threshold(0.5, 1.0, logic="less")
        assert "\033[32m" in result
        assert "0.50" in result

    def test_less_above_threshold_is_red(self) -> None:
        result = colorize_threshold(2.5, 1.0, logic="less")
        assert "\033[31m" in result

    def test_more_above_threshold_is_green(self) -> None:
        result = colorize_threshold(95.0, 50.0, logic="more")
        assert "\033[32m" in result

    def test_ending_appended(self) -> None:
        assert "%" in colorize_threshold(80.0, 90.0, ending="%", precision=0)

    def test_precision_respected(self) -> None:
        result = colorize_threshold(1.23456, 5.0, precision=3)
