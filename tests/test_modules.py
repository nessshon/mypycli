...
class TestModuleBase:
    ...
    @pytest.mark.parametrize("bad_name", ["TonStorage", "ton storage", "-storage"])
    def test_subclass_with_invalid_name_raises(self, bad_name: str) -> None:
        with pytest.raises(TypeError, match=r"\[a-z0-9\]\[a-z0-9_-\]"):
            type("_Bad", (Module,), {"name": bad_name})
...
class TestModuleDB:
    ...
    def test_each_access_is_fresh_instance(self, tmp_path: Path) -> None:
        app = _mk_app(tmp_path)
        mod = _WithDB(app)
        assert mod.db is not mod.db

    def test_external_writes_visible_on_next_access(self, tmp_path: Path) -> None:
        app = _mk_app(tmp_path)
        mod = _WithDB(app)
        assert mod.db.storage_path == "/var/default"
        app.db.set_module_data("with_db", {"storage_path": "/external", "port": 1234})
        assert mod.db.storage_path == "/external"
        assert mod.db.port == 1234
