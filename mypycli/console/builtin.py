    ...
    if args:
        for cmd in commands:
            if str(cmd.name) == args[0] and cmd.children:
                ...
                return
        print(colorize_text(_("no_help_for", name=args[0]), Color.YELLOW))
    else:
...
def cmd_versions(app: Application[Any], _args: list[str]) -> None:
    ...
    rows: list[list[str | ColorText]] = [
        [ColorText(_("table.component"), Color.CYAN), ColorText(_("table.version"), Color.CYAN)],
    ]
...
def cmd_history(app: Application[Any], _args: list[str]) -> None:
    ...
    if not history:
        print(colorize_text(_("no_history"), Color.YELLOW))
        return
...
def cmd_modules(app: Application[Any], _args: list[str]) -> None:
    ...
    rows: list[list[str | ColorText]] = [
        [
            ColorText(_("table.name"), Color.CYAN),
            ColorText(_("table.enabled"), Color.CYAN),
            ColorText(_("table.mandatory"), Color.CYAN),
            ColorText(_("table.capabilities"), Color.CYAN),
        ],
    ]
    rows.extend(
        [
            module.name,
            ColorText(_("table.yes"), Color.GREEN) if module.is_enabled
                else ColorText(_("table.no"), Color.RED),
            ColorText(_("table.yes"), Color.GREEN) if module.mandatory
                else _("table.no"),
...
def cmd_db_get(app: Application[Any], args: list[str]) -> None:
    ...
    """
    if not args:
        print(colorize_text(_("db_get_usage"), Color.RED))
        return
    found, value = app.db.get_by_path(args[0])
    if not found:
        print(colorize_text(_("field_not_found", path=args[0]), Color.RED))
        return
...
def cmd_db_set(app: Application[Any], args: list[str]) -> None:
    ...
    """
    if len(args) < 2:
        print(colorize_text(_("db_set_usage"), Color.RED))
        return
    path = args[0]
    raw = " ".join(args[1:])
    if path == "language":
        ...
        if raw not in available:
            print(colorize_text(
                _("unknown_language", code=raw, available=", ".join(sorted(available))),
                Color.RED,
            ))
            return
    ...
    try:
        ...
        print(colorize_text(_("field_not_found", path=path), Color.RED))
        return
    except ValueError as exc:
        print(colorize_text(_("validation_error", error=format_validation_error(exc)), Color.RED))
        return
    ...
    if path in ("language", "debug"):
        print(colorize_text(_("restart_hint"), Color.YELLOW))
