...
class Console(ConsoleOutput, ConsoleInput):
    ...
    def _resolve_banners(self) -> None:
        ...
        default_goodbye = colorize_text(
            _("goodbye", label=self.app.label), Color.YELLOW
        )
    ...
    def print_update_result(
        ...
        name: str,
        *,
        status: Literal["updated", "up_to_date", "failed"],
        before: str | None = None,
        after: str | None = None,
        name_width: int = 20,
    ) -> None:
        """Print one row of ``update`` output: marker, padded module name, body.

        TTY renders with ANSI colors; non-TTY emits plain text.

        :param status: ``"updated"`` shows ``before → after``; ``"up_to_date"`` and
            ``"failed"`` use the localized message under ``mypycli.update.<status>``.
        :param before: Pre-update version string; required when ``status="updated"``.
        :param after: Post-update version string; required when ``status="updated"``.
        :param name_width: Column width to which ``name`` is left-padded.
        """
        name_padded = name.ljust(name_width)
        tty = sys.stdout.isatty()

        if status == "updated":
            marker, marker_color = "✓", Color.GREEN
            body, body_color = f"{before} → {after}", None
        elif status == "up_to_date":
            marker, marker_color = "•", Color.GRAY
            body, body_color = _("update.up_to_date"), Color.GRAY
        else:
            marker, marker_color = "✗", Color.RED
            body, body_color = _("update.failed"), Color.RED

        if tty:
            marker = colorize_text(marker, marker_color)
            name_padded = colorize_text(name_padded, Color.CYAN)
            if body_color is not None:
                body = colorize_text(body, body_color)
        print(f"{marker} {name_padded} {body}")
    ...
    def _dispatch(self, parts: list[str]) -> None:
        ...
        print(colorize_text(_("unknown_command", name=cmd_name), Color.RED))
    ...
    def _execute_command(self, cmd: Command, args: list[str]) -> None:
        ...
        if cmd.handler is not None:
            try:
                ...
                print(colorize_text(_("command_error", error=err), Color.RED))
        elif cmd.children:
    ...
    def _all_commands(self) -> list[Command]:
        return [
            Command(
                "db",
                description=_("commands.db"),
                children=[
                    Command("show", cmd_db, _("commands.db_show")),
                    Command("get", cmd_db_get, _("commands.db_get"), usage="<field>"),
                    Command("set", cmd_db_set, _("commands.db_set"), usage="<field> <value>"),
                ],
            ),
            *self._commands,
            Command("status", cmd_status, _("commands.status")),
            Command("modules", cmd_modules, _("commands.modules")),
            Command("versions", cmd_versions, _("commands.versions")),
            Command("history", cmd_history, _("commands.history")),
            Command("clear", cmd_clear, _("commands.clear")),
            Command(ColorText("help", Color.GREEN), cmd_help, _("commands.help")),
            Command(ColorText("exit", Color.RED), cmd_exit, _("commands.exit")),
        ]
