...
class ConsoleOutput:
    ...
    def print_panel(
        ...
        *,
        ...
        if header_text:
            ...
            pr(f"{tl}{h} {header_text} {h * fill}{h}{tr}")
        else:
            pr(f"{tl}{h * inner}{tr}")
        ...
        for line in formatted:
            if not line:
                ...
                pr(f"{v}{line}{' ' * pad_right}{v}")
        pr(f"{v}{' ' * inner}{v}")

        if footer_text:
            fill = inner - visible_len(footer_text) - 3
            pr(f"{bl}{h * fill} {footer_text} {h}{br}")
        else:
            pr(f"{bl}{h * inner}{br}")
    ...
    def print_line(width: int = 40) -> None:
        ...
        :param width: Number of dash characters to print.
        """
        print("\u2500" * width)
    ...
    def _print_table_boxed(
        ...
        if footer_text:
            fill = len(top_line) - visible_len(footer_text) - 2
...
def _colorize_scalar(value: object) -> str:
    ...
    :param value: Scalar JSON value to colorize.
    """
    if value is None:
