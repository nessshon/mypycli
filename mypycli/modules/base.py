...
class Module:
    ...
    def __init__(self, app: Application[Any]) -> None:
        ...
        self._db: DatabaseSchema | None = None
    ...
    @property
    def db(self) -> Any:
        """Return the module's typed DB section with auto-save on field assignment.

        Data is persisted under ``db.modules[self.name]``. Nested model fields must
        extend ``DatabaseSchema`` to propagate auto-save.
    ...
    def _init_db(self) -> None:
        ...
        self._db = self.db_schema.model_validate(raw)
        _wire_patch(self._db, self._on_db_patch)
